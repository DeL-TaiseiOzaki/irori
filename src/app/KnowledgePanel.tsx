...
export function KnowledgePanel({
  ...
  return (
    <Dialog
      label={t('資料と成果物', 'Materials & artifacts')}
      className="modal-dialog knowledge-dialog"
      busy={busy}
      onClose={onClose}
    >
      <div className="knowledge-heading">
        <h2>{t('資料と成果物', 'Materials & artifacts')}</h2>
      ...
      <label>
        {t('記録を検索', 'Search records')}
        <input
          type="search"
          aria-label={t('資料・成果物の記録を検索', 'Search material & artifact records')}
          value={query}
          placeholder={t(
            '記録したパス・資料 ID・実行 ID・CLI',
            'Recorded path, material ID, run ID, CLI',
          )}
      ...
      {preview && (
        ...
        >
          ...
          {preview.location.state !== 'unbound' && (
            <p>
              {t('現在の登録先', 'Current registered location')}: {preview.location.current.path}
            </p>
          )}
          <button
            disabled={busy}
