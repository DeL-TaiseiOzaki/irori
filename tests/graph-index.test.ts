...
test('Generating the module writes deterministic tables that the reader loads without a declaration', async (t) => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      ['wiki/グラフ', 'グラフ', 'Knowledge_Base/wiki/グラフ.md'.normalize('NFD'), 'concept'],
    ],
  );
  const linked = view.entities.find((entity) => entity.id === 'wiki/グラフ')!;
  assert.equal((await files.read(space.scopeId, linked.note!)).path, linked.note);
  assert.deepEqual(
...
test('A graph within row limits refuses oversized output before changing any module file', async (t) => {
  const { space, service, write, read } = await fixture(t);
  await service.update(space.scopeId);
  const before = await Promise.all(Object.values(graphIndexFiles).map(read));
  const folder =
    'Knowledge_Base/wiki/projects/knowledge-management/research/2026/notes/architecture-and-implementation/component-guidelines';
  for (let i = 0; i < 1000; i++)
    await write(
      ...
      page(
        ...
        Array.from(
          { length: 9 },
          (_, j) => `{ rel: references, target: page-${(i + j + 1) % 1000}.md }`,
        ),
      ),
    );
  await assert.rejects(() => service.update(space.scopeId), /2 MiB/);
  assert.deepEqual(await Promise.all(Object.values(graphIndexFiles).map(read)), before);
});

test('Corrupt and oversized module files can be regenerated without relaxing declaration or alias guards', async (t) => {
  const { files, space, service, root, write, read } = await fixture(t);
  await service.update(space.scopeId);
  await write(graphIndexFiles.relations, 'sourceId,relation,targetId\nwiki/alone,sees,missing\n');
  await assert.rejects(() => readOntology(files, space.scopeId), /不明/);
  assert.equal((await service.update(space.scopeId)).current, true);
  await write(graphIndexFiles.entities, 'x'.repeat(2 * 1024 * 1024 + 1));
  await write(graphIndexFiles.relations, '\0binary');
  assert.equal((await service.status(space.scopeId)).present, true);
  assert.equal((await service.status(space.scopeId)).current, false);
  const repaired = await service.update(space.scopeId);
  assert.equal(repaired.current, true);
  assert.deepEqual(repaired.written, [graphIndexFiles.entities, graphIndexFiles.relations]);
  assert.equal(await read(graphIndexFiles.entities), expectedEntities);
  assert.equal(await read(graphIndexFiles.relations), expectedRelations);
  if (process.platform !== 'win32') {
    await rm(path.join(root, graphIndexFiles.entities));
    await symlink(
      path.join(root, 'Knowledge_Base/wiki/plain.md'),
      path.join(root, graphIndexFiles.entities),
    );
    await assert.rejects(() => service.update(space.scopeId), /alias/);
    assert.equal(await read('Knowledge_Base/wiki/plain.md'), '# No frontmatter\n');
  }
});

test('Unreadable generated bytes are refused before any module file changes', async (t) => {
  const { files, space, service, write, read } = await fixture(t);
  await service.update(space.scopeId);
  const before = await Promise.all(Object.values(graphIndexFiles).map(read));
  await write(
    'Knowledge_Base/wiki/alone.md',
    page('concept', '"\\0"', ['{ rel: sees, target: broken.md }']),
  );
  await assert.rejects(() => service.update(space.scopeId), /Binary|NUL/);
  assert.deepEqual(await Promise.all(Object.values(graphIndexFiles).map(read)), before);
  await assert.rejects(
    () =>
      files.writeGenerated(
        space.scopeId,
        'Knowledge_Base/too-big.csv',
        '界'.repeat(1024 * 1024),
        null,
      ),
    /2 MiB/,
  );
  await assert.rejects(() => read('Knowledge_Base/too-big.csv'), { code: 'ENOENT' });
});

test('Module note navigation resolves decomposed directories without accepting aliases', async (t) => {
  const actual = 'Knowledge_Base/ガイド/グラフ.md'.normalize('NFD');
  const { files, space, root, service, read } = await fixture(t, [
    [
      'Knowledge_Base/start.md',
      page('concept', 'Start', ['{ rel: uses, target: ガイド/グラフ.md }']),
    ],
    [actual, page('concept', 'Graph')],
  ]);
  await service.update(space.scopeId);
  assert.equal((await read(graphIndexFiles.entities)).includes(actual.normalize('NFC')), true);
  const view = (await readOntology(files, space.scopeId))!;
  const note = view.entities.find((entry) => entry.label === 'Graph')!.note!;
  assert.equal(note, actual);
  assert.equal((await files.read(space.scopeId, note)).text, page('concept', 'Graph'));
  if (process.platform !== 'win32') {
    await rm(path.join(root, actual));
    await symlink(path.join(root, 'Knowledge_Base/start.md'), path.join(root, actual));
