...
validated IPC/storage values. Verification: build/typecheck/format, **141 behaviour
tests (137 passed, four environment-gated skips)**, and all **twelve Electron UI
suites**. `ui-smoke` verifies the expanded menu, computed Markdown font immediately
after selection and the saved choice after two process restarts; the same run
continues through rich editing, save, undo/redo and image paste. No installer,
website or published release changes.
