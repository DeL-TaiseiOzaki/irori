...
- Application source `ac935a9160f5e0a0be3d77a23e71d24073e53fcb` adds local KB text search after shared-configuration documentation commit `9740f98`. It includes earlier record navigation/reconnection and conversation recovery. The implementation is pushed; this following documentation checkpoint records verification. Use current `main`, not a reset to a historical hash. Remote: `https://github.com/DeL-TaiseiOzaki/irori.git`.
- [CI 34810865845](https://github.com/DeL-TaiseiOzaki/irori/actions/runs/34810865845) passes at `ac935a9`: verification (build, behavior/UI and website fixtures) and Linux x64, Windows x64 and Mac arm64 package jobs, including relocated smoke. Local build, 85/88 behavior tests (three optional native-control skips), all ten Electron UI suites, formatting and documentation-link checks pass. Fixtures do not establish installed-device, Google-account or model-turn acceptance. [STATUS](STATUS.md) retains the detailed evidence.
...
Start with actual status/diffs and state the next bounded change and its checks. Create a dedicated feature branch before implementation. Reuse current service boundaries and libraries. Finish a coherent implementation, inspect the diff, run the applicable checks, update the handoff, then commit/push the feature branch and open a PR to `main`. State any remaining device/account dependency precisely.

## Authorization and protected state

- Development, verification, feature-branch commit/push and PR creation are authorized. Every new feature requires a dedicated branch and PR; do not commit or push features directly to `main`. PR creation does not by itself authorize merging. This supersedes older direct-to-main feature instructions. The unsigned Windows testing channel was separately authorized; this is not approval for a general release, Mac publication or unrelated external account changes. Any updated testing installer needs a distinct version, exact verified native artifacts, checksums/notes and a matching manifest/delivery check.
