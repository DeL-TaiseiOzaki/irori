...
For a new session, use the [copyable continuation prompt](HANDOFF-PROMPT.md), prepared at the owner's 2026-09-14 checkpoint request. It separates the newer source from the published installer and names the next D04/D06 work. This prompt/checkpoint follow-up changes documentation only; the last implemented source and verification remain the entries below.
...
Continue development in `/workspace/KB_design/irori`, an independent Git repository on `main`, remote `https://github.com/DeL-TaiseiOzaki/irori.git`. Inspect `git status`, recent commits and current remote release/CI state before making changes. Preserve any work added after this checkpoint. The sibling LayeredKB and claudian template repositories have independent histories and are outside this task.

Read [AGENTS](../AGENTS.md), [README](../README.md), [STATUS](STATUS.md), [host decision](decisions/001-initial-host.md), [confirmed product decisions](decisions/002-release-and-workspace.md), [CHECKPOINT](CHECKPOINT.md) and [RELEASE-PLAN](RELEASE-PLAN.md). The current sections and actual code supersede historical milestone statements. Task-specific references are [DISTRIBUTOR-GOOGLE](DISTRIBUTOR-GOOGLE.md), [CLOUD-SETUP](CLOUD-SETUP.md), [TERMINAL](TERMINAL.md), [PACKAGING](PACKAGING.md), [ACCEPTANCE](ACCEPTANCE.md), [WORKSPACE-DRIVE](WORKSPACE-DRIVE.md) and [ONTOLOGY](ONTOLOGY.md). Parent `../docs/irori/` specifications exist in this workspace; their old Q01/Q02 proposals are superseded by ADR 002.
