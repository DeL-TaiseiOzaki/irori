...
The Google HostAPI intentionally exposes preparation/list/restore only. The transport engine has not been connected to a writable Google capability. OAuth scope, re-consent, binding the native writable transport to the validated account/destination, native Google document conversion, version-aware overwrite, connection-removal recovery UX, queue cancellation/retention, mount refresh, actual network/power-loss testing and provider-side conditional writes remain open. There is no automatic upload on restart and no general cloud-write release claim. Google client registration and existing repository secrets do not need repeating.
