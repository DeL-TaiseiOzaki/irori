...
`knowledge/` under device data contains content-addressed blobs, a per-scope path/UUID index, immutable run-start records, separate completion records and separately appended artifact registrations. A source keeps its ID across edits/restarts. Copies at different paths receive different IDs. The service supports explicit matching-version rebinding after a move; general rename/rebind UI and provider-native file IDs remain open. Prior records retain their original paths and exact bytes even after source deletion. Hash validation detects altered retained bytes.
