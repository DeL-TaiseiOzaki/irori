...
Latest verified implementation: [CI run 34755088101](https://github.com/DeL-TaiseiOzaki/irori/actions/runs/34755088101), commit `3d5678c`, passes clean verification and all Linux x64 / Windows x64 / Mac arm64 jobs for version `0.1.1`. Copied packages pass reload, SDK imports, Japanese note save, CSV graph loading, actual terminal keyboard input/Japanese file creation, bundled rclone startup and normal shutdown. Verification includes 64 passed behavior tests with three optional provider controls skipped, eight UI suites and website fixtures. Windows uses node-pty's official Node-API prebuilds to avoid an unnecessary rebuild against a newer MSVC toolchain. [Preview notes](releases/0.1.1-preview.1.md) identify the published Windows bytes. The paragraphs below retain earlier diagnostic evidence. This remains unsigned engineering evidence; installation on the user's Windows 11/Ryzen 9 and M5 Pro devices, signing and provider acceptance are still open.
